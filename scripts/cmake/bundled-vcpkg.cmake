...
.. command:: enable_vcpkg
  ...
  (default is ``${CMAKE_CURRENT_BINARY_DIR}/bundled-vcpkg``). If ``PATH`` is
  relative, it is interpreted relative to ``CMAKE_CURRENT_BINARY_DIR``. You can
  place vcpkg outside the build directory to skip re-cloning with clean builds.
...
function(_enable_vcpkg)
    ...
    if (DEFINED ARGS_UNPARSED_ARGUMENTS)
        ...
        message(FATAL_ERROR "enable_vcpkg: unrecognized arguments: ${ARGS_UNPARSED_ARGUMENTS_STRING}.")
        return()
    endif()

	if (NOT DEFINED ARGS_PATH)
		set(ARGS_PATH "${CMAKE_CURRENT_BINARY_DIR}/bundled-vcpkg")
	endif()

	if (NOT DEFINED ARGS_VERSION)
		message(FATAL_ERROR "enable_vcpkg: missing value for parameter `VERSION`.")
        return()
	endif()
    # ------------------------------------------------------------------------------------------------------------------
